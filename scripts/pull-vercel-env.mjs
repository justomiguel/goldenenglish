#!/usr/bin/env node
/**
 * Pull env desde Vercel (ver `OUT_FILES`): .vercel/project.json temporal.
...
const OUT_FILES = {
  ...
  liora: path.join(ROOT, ".env.local.liora"),
};
...
function usage() {
  console.error(`pull-vercel-env: ${Object.keys(OUT_FILES).join("|")} [--production|--preview|--development] [-y]
