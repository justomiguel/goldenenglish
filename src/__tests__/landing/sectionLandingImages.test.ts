...
describe("sectionLandingImages", () => {
  it("builds encoded paths per section", () => {
    expect(sectionImageSrc("inicio", "1.png")).toBe("/images/golden/inicio/1.png");
  ...
  it("exports image lists for each block", () => {
    ...
    expect(MODALIDADES_IMAGES.length).toBe(12);
  });

  it("maps collage index to modalidades filenames", () => {
    expect(modalidadesCollageSrc(0)).toBe("/images/golden/modalidades/1.png");
    expect(modalidadesCollageSrc(11)).toBe("/images/golden/modalidades/12.png");
  });
});
