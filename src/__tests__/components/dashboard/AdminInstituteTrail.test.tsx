...
const navState = vi.hoisted(() => ({ pathname: "/en/dashboard/admin/calendar" }));
...
describe("AdminInstituteTrail", () => {
  it("shows Instituto → Calendar on an Instituto child", () => {
    navState.pathname = "/en/dashboard/admin/calendar";
    ...
    expect(screen.getByText(dictEn.dashboard.adminNav.calendar)).toBeInTheDocument();
  });
  ...
  it("hides on the Usuarios list", () => {
    navState.pathname = "/en/dashboard/admin/users";
    const { container } = render(<AdminInstituteTrail locale="en" dict={dictEn.dashboard.adminNav} />);
    expect(container).toBeEmptyDOMElement();
  });
});
