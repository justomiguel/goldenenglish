...
import { LIORA_GALLERY_URLS } from "@/lib/landing/lioraGalleryImages";

export interface LioraLandingGalleryProps {
  dict: Dictionary;
}

export function LioraLandingGallery({ dict }: LioraLandingGalleryProps) {
  const titleId = useId();
  const [open, setOpen] = useState(false);
  const [index, setIndex] = useState(0);
  const [slideLoading, setSlideLoading] = useState(true);
  const images = LIORA_GALLERY_URLS;
  const total = images.length;
  const safeTotal = Math.max(total, 1);
  const preview = images.slice(0, 3);

  const t = (key: string) => marketingLandingCopy(dict, "liora", `galeria.${key}`);
  ...
  return (
    ...
    >
      <div className="text-center">
        <p className="liora-kicker text-[var(--liora-rose-deep)]">{t("kicker")}</p>
      ...
      <div className="mx-auto mt-12 grid max-w-5xl gap-6 sm:grid-cols-3 md:gap-8">
        {preview.map((src, i) => (
          <button
            key={src}
            type="button"
            className="liora-gallery-frame group relative aspect-[3/4] w-full cursor-pointer bg-white p-0 text-left transition-[box-shadow,transform] hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--liora-rose-deep)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--liora-blush)] active:scale-[0.99]"
      ...
      <div className="mt-12 flex justify-center">
        <Button
          type="button"
          variant="secondary"
          className="min-h-[44px] gap-2 rounded-full border border-[var(--liora-rose-deep)] bg-transparent px-8 py-3 text-xs font-medium uppercase tracking-[0.25em] text-[var(--liora-rose-deep)] transition-colors hover:bg-[var(--liora-rose-deep)] hover:text-white"
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          >
            {slideLoading ? (
              ...
              >
                <Loader2
                  className="h-10 w-10 shrink-0 animate-spin text-[var(--liora-rose-deep)]"
                  aria-hidden
                  strokeWidth={2}
                />
