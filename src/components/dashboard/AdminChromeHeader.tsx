...
export function AdminChromeHeader({ locale, brand, dict }: AdminChromeHeaderProps) {
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-[var(--layout-max-width)] items-center justify-between gap-3 px-4 py-3.5 md:px-6">
        <Link
          href={`/${locale}/dashboard/admin`}
          className="group flex min-w-0 max-w-[min(100%,28rem)] flex-1 items-center gap-3 rounded-[var(--layout-border-radius)] outline-none ring-[var(--color-primary)] transition hover:opacity-95 focus-visible:ring-2 focus-visible:ring-offset-2 md:max-w-none"
        >
          <div className="shrink-0 rounded-[var(--layout-border-radius)] bg-[var(--color-background)] p-1.5 shadow-sm ring-1 ring-[var(--color-border)] transition group-hover:ring-[var(--color-accent)]/40">
            <Image
              ...
              width={44}
              height={44}
              className="block h-10 w-10 rounded-[var(--layout-border-radius)] object-contain"
              priority
            />
          </div>
          <div className="min-w-0 flex-1 text-left">
            <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
              <span className="font-display text-lg font-semibold tracking-tight text-[var(--color-primary)] md:text-xl">
                {brand.name}
              </span>
              <span className="rounded-full border border-[var(--color-border)] bg-[var(--color-muted)]/70 px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wider text-[var(--color-muted-foreground)]">
                {labels.badge}
              </span>
            </div>
