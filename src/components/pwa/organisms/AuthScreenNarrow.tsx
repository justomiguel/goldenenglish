...
interface AuthScreenNarrowProps {
  brand: BrandPublic;
  dict: Dictionary;
  locale: string;
...
/**
 * Touch-first auth chrome with safe-area padding for installed PWA. Shared
 * by `/login`, `/forgot-password` and `/reset-password` so the brand header,
 * navigation and spacing stay aligned on phones.
 */
export function AuthScreenNarrow({
  brand,
  dict,
  locale,
  surface,
  children,
}: AuthScreenNarrowProps) {
