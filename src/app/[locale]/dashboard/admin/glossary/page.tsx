...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: dict.dashboard.adminGlossaryPage.metaTitle,
    robots: { index: false, follow: false },
  };
}

export default async function AdminGlossaryPage({ params }: PageProps) {
  const { locale } = await params;
  try {
    await assertAdmin();
  } catch (err) {
  ...
  return (
    <AdminGlossaryScreen
      pageDict={dict.dashboard.adminGlossaryPage}
      glossaryDict={dict.dashboard.adminHelpGlossary}
    />
  );
}
