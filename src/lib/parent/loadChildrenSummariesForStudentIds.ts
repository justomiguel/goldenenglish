...
/** Loads parent-style child summaries for an explicit list of student profile ids. */
export async function loadChildrenSummariesForStudentIds(
  ...
  const teacherIdByStudentId = await loadTeacherIdByStudentId(supabase, uniqueIds);
  ...
  for (const p of profiles ?? []) {
    ...
    let lastPublishedGrade: ParentChildSummary["lastPublishedGrade"] = null;
