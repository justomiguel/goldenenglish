...
async function loadMonthAttendanceByEnrollment(
  ...
  for (let i = 0; i < unique.length; i += chunkSize) {
    ...
    if (error) {
      logSupabaseClientError("loadRecentAttendanceForStudentIds:monthAttendance", error, {
...
/** Recent attendance model for explicit student ids (parent wards or self). */
export async function loadRecentAttendanceForStudentIds(
  supabase: SupabaseClient,
  studentIds: string[],
  ...
  if (profErr) {
    logSupabaseClientError("loadRecentAttendanceForStudentIds:profiles", profErr, {});
    return empty();
  }
  ...
  if (enrErr) {
    logSupabaseClientError("loadRecentAttendanceForStudentIds:enrollments", enrErr, {});
  ...
  const sectionSummaries: ParentAttendanceSectionSummary[] = buildParentAttendanceSectionSummaries(
    summaryMeta,
